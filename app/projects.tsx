...
export default function ProjectsScreen() {
  ...
  return (
    <ResponsiveScreen keyboard>
      <ResponsiveContent bottomPadding={70}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 8 }}>
          ...
          <Text numberOfLines={1} style={{ flex: 1, textAlign: "center", fontSize: isNarrow ? 22 : 26, fontWeight: "800", color: colors.text }}>
            Product Projects
          </Text>
          <Pressable onPress={() => setShowCreate((v) => !v)} style={{ padding: 8, marginRight: -8 }}>
        ...
        {showCreate ? (
          <View style={{ marginTop: 20, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, borderRadius: 20, padding: isNarrow ? 14 : 16, gap: 12 }}>
            <Text style={{ color: colors.text, fontSize: 18, fontWeight: "700" }}>Naya Product Project</Text>
            <TextInput value={name} onChangeText={setName} placeholder="Project name *" placeholderTextColor={colors.muted}
              style={{ color: colors.text, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 13 }} />
            <TextInput value={product} onChangeText={setProduct} placeholder="Product name" placeholderTextColor={colors.muted}
              style={{ color: colors.text, borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 13 }} />
            <TextInput value={description} onChangeText={setDescription} placeholder="Product description / offer / audience"
              placeholderTextColor={colors.muted} multiline
              style={{ color: colors.text, minHeight: 90, textAlignVertical: "top", borderWidth: 1, borderColor: colors.border, borderRadius: 12, padding: 13 }} />
            <Pressable disabled={busy || !name.trim()} onPress={() => void submit()}
              style={{ backgroundColor: colors.primary, padding: 14, borderRadius: 13, alignItems: "center", opacity: busy || !name.trim() ? 0.45 : 1 }}>
        ...
        <View style={{ gap: 12, marginTop: 22 }}>
          {projects.map((project) => (
            <Pressable key={project.id} onPress={() => router.push({ pathname: "/project/[id]", params: { id: project.id } })}
              style={{ borderWidth: 1, borderColor: colors.border, backgroundColor: colors.card, borderRadius: 18, padding: isNarrow ? 14 : 17 }}>
