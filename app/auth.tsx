...
export default function AuthScreen() {
  ...
  const { isCompactHeight, isNarrow } = useResponsiveLayout();
  ...
  async function submit() {
    if (!email.trim() || !password) return setError("Email aur password required hain.");
    if (mode === "signup" && !name.trim()) return setError("Naam required hai.");
    ...
    try {
      const result = mode === "signup"
        ...
        : await authClient.signIn.email({ email: email.trim().toLowerCase(), password, rememberMe: true });
  ...
  return (
    <ResponsiveScreen keyboard>
      <ResponsiveContent bottomPadding={24} style={{ flexGrow: 1, justifyContent: isCompactHeight ? "flex-start" : "center", paddingTop: isCompactHeight ? 18 : 28 }}>
        <View style={{ width: "100%", maxWidth: 520, alignSelf: "center" }}>
          <Text style={{ color: colors.text, fontSize: isNarrow ? 30 : 34, fontWeight: "800" }}>AdBrain</Text>
          <Text style={{ color: colors.muted, fontSize: 16, marginTop: 8 }}>
            {mode === "signin" ? "Sign in to AdBrain One" : "Create your AdBrain account"}
          </Text>

          <View style={{ gap: 12, marginTop: isCompactHeight ? 22 : 34 }}>
            {mode === "signup" ? (
            ...
            <TextInput value={password} onChangeText={setPassword} secureTextEntry
              placeholder="Password (8+ characters)" placeholderTextColor={colors.muted}
              style={{ backgroundColor: colors.card, color: colors.text, borderWidth: 1, borderColor: colors.border, borderRadius: 14, paddingHorizontal: 15, paddingVertical: 14, fontSize: 16 }} />
            ...
            <Pressable disabled={busy} onPress={() => void submit()}
              ...
              {busy ? <ActivityIndicator color={colors.background} /> : (
                <Text style={{ color: colors.background, fontWeight: "700", fontSize: 16 }}>{mode === "signin" ? "Sign in" : "Create account"}</Text>
              )}
            </Pressable>
          </View>
          ...
          <Pressable onPress={() => { setMode(mode === "signin" ? "signup" : "signin"); setError(""); }}
            ...
            <Text style={{ color: colors.text }}>{mode === "signin" ? "New here? Create account" : "Already have an account? Sign in"}</Text>
          </Pressable>
        </View>
      </ResponsiveContent>
    </ResponsiveScreen>
